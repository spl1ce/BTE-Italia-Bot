...
class Reviews(commands.Cog):
    ...
    def store(self, review):

    async def accept(self, ctx, guild, message):
    ...
    async def reject(ctx, guild, message):
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if payload.channel_id == 867045228543606854:
            ...
            if payload.emoji == verify_emoji:
                ...
                await self.accept(ctx, guild, message)

            elif payload.emoji == '❌':

                await self.reject(ctx, guild, message)
